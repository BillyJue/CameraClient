...
typedef struct tagVideoCapList
{
	...
	char szEncorderComplexityCap[64];	// 编码复杂度能力
	TVIDEOFRAMECAPLIST tVideoCapList[5]; //帧率能力链表
...
{
	TVIDEOCAPLIST	tVideoCapList[10];	// 最多10个
	TVIDEOENC		tVideoEnc;
...
{
	u8	byVideoSource;
	u8	bySreamId;		// 主码流(1),辅码流(2),三码流(3),四码流(4)
	TVIDEOENC	tVideoEnc;
...
{
	char szircutfiltertype[17];		// 转换类型
...
typedef struct tagPtzCoorDinate
{
	...
	int  nVertical;            // 垂直坐标 min="-1500" max="9000"
